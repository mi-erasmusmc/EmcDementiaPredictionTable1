...
The setup script below is used to install the **EmcDementiaPredictionTable1** package. You will need to modify this setup script as follows:
                                                                                              
- Set the `projectRootFolder` variable to the directory specific to your environment. In this example we are using `C:/EmcDementiaPredictionTable1`. This root folder will serve a few purposes:
...
projectFolder <- "C:/EmcDementiaPredictionTable1"
setwd(projectFolder)
                                                                                                 
# Download the lock file:
download.file("https://raw.githubusercontent.com/ohdsi-studies/EmcDementiaPredictionTable1/master/renv.lock", "renv.lock")
...
You can safely continue by pressing 'y' after this prompt since the renv.lock file is downloaded from the **EmcDementiaPredictionTable1** GitHub code repository. Once the installation is complete, you may need to restart R (if you are working outside of RStudio) and you should see this message:

````
Project 'C:/EmcDementiaPredictionTable1' loaded. [renv 0.x.y]
````
